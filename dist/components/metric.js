...
var Metric = function (_Component) {
  ...
  Metric.prototype.componentDidMount = function componentDidMount() {
  ...
  Metric.prototype.componentWillUnmount = function componentWillUnmount() {
    var resize = (0, _reactDom.findDOMNode)(this.refs.resize);
    if (!resize) return;
    resize.removeEventListener('resize', this.handleResize);
  };

  Metric.prototype.calculateCorrdinates = function calculateCorrdinates() {
  ...
  Metric.prototype.componentDidUpdate = function componentDidUpdate() {
  ...
  Metric.prototype.calcDimensions = function calcDimensions(el, scale) {
    var newWidth = Math.floor(el.clientWidth * scale);
    var newHeight = Math.floor(el.clientHeight * scale);
    return [newWidth, newHeight];
  };

  Metric.prototype.handleResize = function handleResize() {
    // Bingo!
    var newState = this.calculateCorrdinates();
    this.setState(newState);
  };
...
Metric.propTypes = {
  ...
  secondary: _react.PropTypes.object,
